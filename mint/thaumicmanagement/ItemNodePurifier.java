...
{	
    ...
    {		
    	if (player.isSneaking() == false)
    	{
        	...
        	{
        		player.addChatMessage("Node " + ThaumcraftApi.getClosestAuraWithinRange(world, player.posX, player.posY, player.posZ, ConfigHelper.ToolRange) + " is within range.");
        	}
    	...
    	{
    		int NodeKey = ThaumcraftApi.getClosestAuraWithinRange(world, player.posX, player.posY, player.posZ, ConfigHelper.ToolRange);
    		AuraNode KeyedNode = ThaumcraftApi.getNodeCopy(NodeKey);
    		
    		if (KeyedNode != null)
    		{
    			//this.purify(itemstack, KeyedNode);
    			ThaumcraftApi.queueNodeChanges(itemstack.stackTagCompound.getInteger("NodeKey"), (KeyedNode.baseLevel - KeyedNode.level), 0, false, null, 0, 0, 0);
    		}
    		
    		else
    		{
        		player.addChatMessage("You can't purify a nonexistant node...");
    		}
    	}
